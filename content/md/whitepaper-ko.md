...
- `Packet`: an `IBCPacket`:
  - `Header`: an `IBCPacketHeader`:
    ...
    - `Type`: "coin"
    - `MaxHeight`: 350
  - `Payload`: 는 앱-The same bytes of a "coin" payload 생략형
...
- `Packet`: an `IBCPacket`:
  - `Header`: an `IBCPacketHeader`:
    ...
    - `Type`: "coin"
    - `MaxHeight`: 350
  - `PayloadHash`: AckThe hash bytes of the same "coin" payload 완결된
...
- `Packet`: an `IBCPacket`:
  - `Header`: an `IBCPacketHeader`:
    ...
    - `Type`: "coin"
    - `MaxHeight`: 350
  - `PayloadHash`: 패킷 전 The hash bytes of the same "coin" payload 가정할
...
IAVL+ 데이터 구조의 목적은 애플리케이션 상태의 키-값 쌍들에 영구 저장(persistent storage)을 제공하는 것이다. 그렇게 하여 결정론적 머클 루트 해시를 효율적으로 계산한다. 트리는 (log(n))의 새 노드들을 만들어서 다시 균형을 잡는다. 기존 AVL 알고리즘에서는 의 변형(variant)을 사용하여 밸런싱되며(balancing), 모든 연산(operations)은 O(log(n))이다.
...
AVL+ 트리는 이더리움의 ariant) 사용이 가능해지면, 이더리움의 패트리샤 트라이 같은 효율적 머클 트리들을 추가로 지원할 것이다.

### 와 유사하다. 장단점이 존재한다. 키들은 IAVL+ 트리에 삽입되기 전에 해싱될 필요가 없고, 따라서 키 공간(key space)에서의 빠른 순서반복(ordered iteration)을 제공한다. 로직의 구현이 간편하고 내부 노드(inner nodes)와 잎 노드(leaf nodes)만 필요로 한다. 머클 증명은 일반적으로 짧고 균형 잡힌 이진 트리이다. 반면에 IAVL+ 트리의 머클 루트는 업데이트의 순서(order of updates)에 의존한다.
...
- rding, especially under the TMSP section of [SkuChain](https://www.skuchain.com/) provided much help in formatting and
...
- [Andrew Miller](http://soc1024.com/) of r feedback on consensus and wording

* Also thanks to [Bill Gleim](https://githu for feedback on consensus

- Han](http://www.seunghwanhan.com) for for feedback on consensus and wording
- Also thanks to organization here for your contributi and

## 인용 (Citations) ################ for various contributions.
...
- [3] Bitcoin: https://github.com/ethereum/wi
- [4] ZeroCash: tps://download.slock.it/public/DA
- [5] Ethereum: Witness: https://github.com/bitcoin/bips/blob/ma
- [6] TheDAO: G: https://arxiv.org/pdf/1510.02037v2.pdf

* [7] Lig

- [7] Segregated Witness: ttps://lightning.network/lightning-network-paper-DRAFT-0.5.pdf
- [8] BitcoinNG: https://github.com/tendermint/tenderm
- [9] Lightning Network: https://groups.csail.mit.edu/tds/papers/Lynch/jacm85.pdf

* [10]

- [1] Tendermint: tps://blog.ethereum.org/2014/01/15/slasher-a-
- [1] FLP Impossibility: sail.mit.edu/papers/osdi99.pdf

* [12] BitShares: https:/

- [12] Slasher: : https://bitshares.org/technology/delegated-proof-of-stake-consensus/

* [13] Ste

- [13] PBFT: r: https://www.stellar.org/papers/stellar-
- [14] BitShares: : https://interledger.org/rfcs/0001-interledger-architecture/

* [15]

- [15] Stellar: s: https://blockstream.com/sidechains.pdf

* [16] Casper: http

- [16] Interledger: ://blog.ethereum.org/2015/08/01/introducing-casper-friendly
- [17] Sidechains: //github.com/tendermint/tmsp

* [18] Et

- [18] Casper: Sharding: https://github.com/ethereum/EIPs/issues/53

* [19] LibSwift:

- [19] TMSP: ft: http://www.ds.ewi.tudelft.nl/f
- [20] Ethereum Sharding: csail.mit.edu/tds/papers/Lynch/jacm88.pdf

- [21] LibSwift: t Security: https://en.bitcoin.it/wiki/Thin_Client_Security

* [22] Ethereum 2.0 Mau

- [22] DLS: eum 2.0 Mauve Paper: http://vitalik.ca/files/mauve_pape
- [##] Thin Client Security:

* [https://www.docdroid.net/ec7xGzs/314477721-

- [# ] Ethereum 2.0 Mauve Paper: ttps://www.docdroid.net/ec7xGzs/31447772
