...
button.tm-btn(:type='type' v-else)
  span(:class='btnClass')
    i(v-if='icon', :class="'tm-btn__icon material-icons'" aria-hidden='true') {{ icon }}
    img(v-if='!icon && img', :src="img" :class="'tm-btn__img'" aria-hidden='true' alt="Button icon")
    span.tm-btn__value(v-if='value') {{ value }}
</template>
...
.tm-btn__container.tm-btn--transparent
